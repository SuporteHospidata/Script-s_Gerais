...
pg_dumpall.exe -h %HOST% -p %PORT% -U postgres -v -g > %DIR_OUT%\Usuarios.sql

pg_dump.exe -h %HOST% -p %PORT% -U postgres -v -C %CLIENT% > %DIR_OUT%\Backup%CLIENT%%date:~6,4%%date:~3,2%%date:~0,2%.sql
